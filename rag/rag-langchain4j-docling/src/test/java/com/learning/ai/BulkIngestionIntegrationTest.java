...
class BulkIngestionIntegrationTest extends AbstractIntegrationTest {
    ...
    void verifyVectorStoreMetadata() throws Exception {
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
                "SELECT metadata FROM vector_store WHERE metadata->>'source_filename' = ?",
                "layout-sample.pdf"
        );
        ...
        for (Map<String, Object> row : rows) {
            ...
            Number pageNumber = (Number) metadata.get("page_number");
            if (pageNumber != null) {
                if (pageNumber.intValue() == 1) foundPage1 = true;
                if (pageNumber.intValue() == 2) foundPage2 = true;
            }
            
            String elementType = (String) metadata.get("element_type");
            if ("table".equals(elementType)) foundTable = true;
            if ("heading".equals(elementType) && metadata.containsKey("section_path") && metadata.get("section_path") != null) {
                foundHeadingWithSectionPath = true;
            }
        }
        
        assertThat(firstDocumentId).isNotNull();
        assertThat(foundPage1).isTrue();
        assertThat(foundPage2).isTrue();
    ...
    void retrieveTableContent() {
        ...
        RetrievalResponse response = given()
        ...
        assertThat(metadata).containsEntry("page_number", 1);
